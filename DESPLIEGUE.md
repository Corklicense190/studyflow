...
2. **Crear las tablas.** En el **SQL Editor**, pega el contenido de [`db/esquema.sql`](db/esquema.sql) y ejecútalo (*Run*).
   **Al actualizar la aplicación:** cuando una versión nueva agrega columnas o tablas, `db/esquema.sql` las incluye (con `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`). Vuelve a correr el archivo completo en el SQL Editor **ANTES de desplegar** la versión nueva: es seguro repetirlo, no borra ni cambia datos. Si se despliega primero, las rutas que usan la columna nueva responden error 500 hasta que se aplique. (La versión con colores por tipo agrega `color_examen`, `color_evidencia` y `color_tarea` a `configuracion`; la versión con notas agrega `notas` a `entregables`; la versión con perfil agrega la tabla `perfiles`.)
3. **Cerrar el acceso público a las tablas.** En el mismo editor ejecuta [`db/supabase-seguridad.sql`](db/supabase-seguridad.sql) (también hay que **volver a correrlo** cuando se agrega una tabla nueva, como `perfiles`: es seguro repetirlo). Supabase publica por defecto las tablas mediante una API REST protegida solo por la llave `anon` (pública por diseño). StudyFlow no usa esa API, así que se bloquea (RLS activado sin políticas + permisos revocados). **No te saltes este paso**: sin él, cualquiera con la llave `anon` podría leer la tabla de usuarios.
